...
import {
  calcBitgetLongScore,
  calcBitgetLongTradeSetup,
  calcBtcCorrelation,
  calcLongFrWeeklyCost,
  calcLongRecommendedLev,
  calcTrendDir,
} from "@/app/lib/bitgetLongScorer";
import type { BitgetLongCandidate, TrendDir } from "@/app/lib/bitgetLongScorer";
...
async function analyzeCandidate(meta: CandidateMeta, btcCloses: number[]): Promise<BitgetLongCandidate | null> {
  ...
  const { score, breakdown, trendAlignment, rsi } = calcBitgetLongScore(
    athDropPct, fr, volumeChangeRatio, oiRatio, trendH1, trendH4, trendD1, priceChange7d, btcCorr, kline4h.closes,
  );

  const tradeSetup     = calcBitgetLongTradeSetup(price, kline4h.highs, kline4h.lows);
  const frWeeklyCost   = calcLongFrWeeklyCost(fr);
  const recommendedLev = calcLongRecommendedLev(athDropPct, trendAlignment, fr);
...
export async function GET(_req: NextRequest) {
  // ── Stage 0: all tickers ─────────────────────────────────────────────────
  let tickers = getCached("bitgetlong:tickers");
  if (!tickers) {
    ...
    tickers = res.data;
    setCached("bitgetlong:tickers", tickers);
  }

  const totalPairs = (tickers as unknown[]).length;

  // ── Stage 0.5: BTC 1D klines ─────────────────────────────────────────────
  let btcCloses: number[] = getCached("bitgetlong:btcCloses") ?? [];
  if (btcCloses.length === 0) {
    ...
    if (btcRes?.data) {
      ...
      setCached("bitgetlong:btcCloses", btcCloses);
    }
  }

  // ── Stage 1: pre-filter (prioritise negative FR + low OI) ────────────────
  ...
  for (const t of tickers as any[]) {
    ...
    // Quick pre-score: favour negative FR and low OI
    const quickScore = (fr !== null && fr < 0 ? 3 : 0) + (quickOiRatio <= 0.5 ? 2 : quickOiRatio <= 1.0 ? 1 : 0);
  ...
  console.log(`[bitget-long-scan] Stage1: ${stage1Passed} → ${targets.length} targets`);

  // ── Stage 2: kline analysis ───────────────────────────────────────────────
  const results: BitgetLongCandidate[] = [];
  let fetched = 0, failed = 0;
  const DEADLINE = Date.now() + 100_000;

  for (let i = 0; i < targets.length; i += BATCH) {
    if (Date.now() >= DEADLINE) {
      console.warn(`[bitget-long-scan] Stage2 deadline at ${i}/${targets.length}`);
      break;
    }
  ...
  const sorted = results.sort((a, b) => b.longScore - a.longScore);
  const top50  = sorted.slice(0, 50);
  console.log(`[bitget-long-scan] Stage2: fetched=${fetched}, failed=${failed}, passed=${results.length}, returned=${top50.length}`);

  // ── Stage 3: L/S ratio for top 30 ────────────────────────────────────────
  ...
  console.log(`[bitget-long-scan] Stage3: L/S fetched=${lsFetched}/${top30.length}`);
